...
{	
    ...
    public function init_delete()
    {
        ...
        {
            $this->mydb1->trans_commit();
            $this->delete_generator_password($id);
            $this->delete_user_account($id);
            $this->model_message->messege_proses('Data Berhasil dihapus.','delete',$url,'fa-check-square-o','success');
            return TRUE;
        }
    }

    public function delete_generator_password($id)
    {
        $this->mydb1->trans_start();

        $this->mydb1->where('id_user',$id);
        $this->mydb1->delete('generator_pass');
    ...
    public function delete_user_account($id)
    {
        ...
        $this->mydb1->where('id_user',$id);
        $this->mydb1->delete('_users_account');
